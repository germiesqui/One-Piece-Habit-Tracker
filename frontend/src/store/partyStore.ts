...
export const usePartyStore = create<PartyState>((set, get) => ({
  ...
  checkAndAdvanceArc: async (partyId) => {
    ...
    // ACTIVE: check if arc bar full → try boss unlock
    if (arcProgress.status === 'active') {
      ...
      if (!powerMet || !bountyMet) return { event: null } // bar full, stats not met yet
 
      // No boss arc (transition) — skip straight to next
